...
class Payments_m extends CI_Model {
  ...
  function send_email($to_email, $filename)
  {
    $from_email = 'sistemaenlinea@gmail.com'; // cambiar esto
    $subject = 'boleta de pago';
    $message = 'Estimado usuario,<br /><br />adjuntamos su boleta de pago en formato pdf para que pueda descargarlo.<br /><br />';

    $this->email->from($from_email, 'sistema de prestamos');
    $this->email->to($to_email);
    $this->email->subject($subject);
    $this->email->message($message);
    $this->email->attach(FCPATH.$filename.'.pdf');
  ...
  {
    $this->db->select("l.id as loan_id, l.customer_id, c.dni, c.email, concat(c.first_name, ' ', c.last_name) AS cst_name, l.credit_amount, l.payment_m, co.name as coin_name");
